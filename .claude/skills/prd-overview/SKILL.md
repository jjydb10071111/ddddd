...
수강길잡이는 대학생의 수강 의사결정(리뷰 탐색·검색·커리큘럼 설계)을 돕는 AI 기반 서비스입니다. 전체 PRD 원문은 이 세션의 대화 기록에 있지만, 새 세션이나 서브에이전트에서는 아래 요약이 유일한 근거가 됩니다 — 세부 구현 시 반드시 담당 스킬/에이전트로 더 들어가서 확인하세요.
