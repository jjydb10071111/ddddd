...
## 추가 요청 기능 (PRD F1~F4 범위 밖, 사용자 직접 요청)

Sprint 0~5 완료 후 사용자가 추가로 요청해 구현한 기능들 — PRD/스프린트 계획에는 없지만
동일한 아키텍처 원칙(파사드 패턴, Neon 연동, 실 데이터 검증)을 따라 구현했다.

- **회원가입(학교 이메일/휴대폰/개인정보 동의)**: `app/api/auth/register`가 학번·비밀번호·
  학교 이메일(.ac.kr)·휴대폰 번호·개인정보 동의를 받아 계정을 생성한다. 기존에는 로그인
  라우트가 존재하지 않는 학번이면 자동으로 계정을 만들었는데, 회원가입 폼이 생기면서
  이 자동 생성을 제거했다 — 이제 로그인은 가입된 계정만 인증하고, 미가입 학번은 401과
  함께 회원가입 안내 메시지를 준다. `users.phoneNumber`(unique)와
  `users.privacyConsentedAt`(가입 시각 기록) 컬럼 추가.
- **장바구니 + 나의 시간표**: 위 "장바구니와 나의 시간표" 관련 커밋 참고
  (`app/api/cart`, `app/cart`, `app/timetable`, `lib/timetable/parse-schedule.ts`).
- **상단바 네비게이션**: 로그인 시에만 나의 시간표/강의평 보기/장바구니 링크 노출,
  기존 "커리큘럼 설계" 링크는 "AI 커리큘럼"으로 라벨만 변경.

두 차례 모두 `vercel deploy --prod`로 https://sprint0-infra.vercel.app 에 재배포하고
라이브로 검증했다.
